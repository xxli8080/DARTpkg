...
#'
#' @import ape qlcMatrix xtable cubature stats
#' @param grids (vector) vector of maximum distance on layers 2,...,L. The maximum distance on the first layer is always 0. 
#' @param Dist0 (matrix) Optional distance matrix. Default is NULL.
#' @param Mgroup (integer) maximum cardinality of the child node set of the nodes in the aggregation tree.
#'
#' @return A list including constructed tree and distance matrix: 1. Llist: (list of matrixs) Aggregation tree presented in the form of matrix list; 2, Dist0: (matrix) distance matrix
#' @export
A.tree.mult <- function(grids,Dist0,Mgroup=3){
...
#'
#' @import ape qlcMatrix xtable cubature stats
#' @param alpha (numeric) Vector of desired FDR
#' @param Llist (list of matrixes) Aggregation tree presented in the form of matrix list.
#' @param Dist0 (matrix) Distance matrix
#' @param T1 (vector) Input Z-values transformed from p-values
#'
